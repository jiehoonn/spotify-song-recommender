...
- **popularity**: Spotify popularity score (0–100, normalized for modeling)
- **lastfm_tags**: List of top tag names for the track from Last.fm
- **lastfm_tag_counts**: JSON mapping of tag names to their count (number of users who applied each tag)
...
- **id**: Primary key
- **song_id**: Foreign key to `songs.id`
- **embedding**: Dense vector (array of floats) representing the song’s lyrics, generated by Sentence-BERT
- **model_name**: Name of the embedding model used (e.g., `all-mpnet-base-v2`)
